...
class AgentOrchestrator:
    ...
    async def claude_call(self, system_prompt, user_message, agent_name="Claude"):
    ...
    async def natty_get_news(self, stocks, days=1):
    ...
    async def num_analyze_stocks(self, news_data, stocks):
...
Provide analysis in JSON format."""
                ...
                try:
                    response = await self.claude_call(system_prompt, user_message, "หนุ่ม")
    ...
    async def mud_cross_validate(self, analysis_results):
...
Check if signal, confidence, and S-levels are consistent."""
                ...
                try:
                    response = await self.claude_call(system_prompt, user_message, "มด")
    ...
    async def harry_monitor_portfolio(self, analysis_results):
    ...
    async def jen_generate_report(self, analysis_results, portfolio_status):
...
Create professional Thai report."""
            ...
            response = await self.claude_call(system_prompt, user_message, "เจน")
    ...
    async def nan_qa_check(self, analysis_results, report):
...
Check if everything is correct and consistent."""
            ...
            response = await self.claude_call(system_prompt, user_message, "นน")
    ...
    async def kao_retry(self, agent_name, error_msg, attempt=1):
...
Suggest retry strategy."""
            ...
            response = await self.claude_call(system_prompt, user_message, "เก้า")
    ...
    # ==================== MAIN WORKFLOW ====================
    async def run_workflow(self, stocks=None, portfolio=None, include_weekend=False):
        ...
        try:
            ...
            news_data = await self.natty_get_news(stocks, days=3 if include_weekend else 1)
            
            # Step 2: หนุ่ม Analyze
            analysis_results = await self.num_analyze_stocks(news_data, stocks)
            
            # Step 3: มด Cross-Validate
            validated_results = await self.mud_cross_validate(analysis_results)
            
            # Step 4: แฮรี่ Monitor Portfolio
            portfolio_status = await self.harry_monitor_portfolio(validated_results)
            
            # Step 5: เจน Generate Report
            report = await self.jen_generate_report(validated_results, portfolio_status)
            
            # Step 6: นน QA Check
            qa_result = await self.nan_qa_check(validated_results, report)
            
            # Step 7: Decision (PASS/REJECT)
            if qa_result.get('status') == 'PASS':
                self.log_action("SYSTEM", "✅ Workflow PASSED - Updating database...", "SUCCESS")
                await self._update_database(validated_results)
                ...
                await self.kao_retry("workflow", qa_result.get('issues', ['Unknown error'])[0], 1)
    ...
    async def _update_database(self, analysis_results):
